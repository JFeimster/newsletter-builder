...
function ProjectsView({result,projects,status,account,save,openPlan,duplicate,remove,refresh,continueBuilding}:{result:Result|null;projects:Result[];status:string;account:Account|null;save:()=>void;openPlan:(r:Result)=>void;duplicate:(r:Result)=>void;remove:(id:string)=>void;refresh:()=>void;continueBuilding:()=>void}) {
  ...
  return <section className="workspace"><SectionHead eyebrow={status==="session"?"SESSION WORKSPACE":"DURABLE WORKSPACE"} title={status==="session"?"SESSION-ONLY PROJECT.":"SAVED PUBLICATIONS."} deck={status==="session"?"This Vercel mirror keeps work in the active browser session. Export before leaving, or use the full Sites version for durable projects.":"Signed-in projects are stored durably with a new immutable version created on every save."} action={<div className="project-actions">{status!=="session"&&<button className="button button-paper" onClick={refresh}>REFRESH</button>}<button className="button button-primary" disabled={!result||status==="session"} onClick={save}>{status==="session"?"SESSION ONLY":"SAVE CURRENT PROJECT"}</button></div>}/>
    {status==="session"&&<div className="signin-card"><span>VERCEL MIRROR · SESSION ONLY</span><h2>Build here. Export here. Save durably on the full Sites version.</h2><p>This deployment does not simulate an account or database. Your inputs and generated plan remain in this browser tab’s session storage.</p><div className="signin-benefits"><b>SAFE OPTIONS</b><ul><li>Export the complete strategy, JSON, Markdown, or calendar CSV</li><li>Keep editing while this browser session remains open</li><li>Use the Sites version for ChatGPT sign-in and durable project history</li></ul></div><div className="signin-actions"><a className="button button-primary" href="https://newsletter-builder.feimster.chatgpt.site/">OPEN FULL SITES VERSION →</a><button className="button button-paper" onClick={continueBuilding}>CONTINUE BUILDING</button></div></div>}
    {status==="signin"&&<div className="signin-card"><span>SIGN IN ONLY WHEN THE WORK IS WORTH KEEPING</span><h2>Your strategy can stay anonymous. Your publication library cannot.</h2><p>Keep building without an account, or sign in with ChatGPT when you want durable projects, cross-session access, duplication, and version history.</p><div className="signin-benefits"><b>WHAT SIGN-IN UNLOCKS</b><ul><li>Durable project storage</li><li>Immutable save history</li><li>Reopen and duplicate publications</li><li>Delete and manage owned projects</li></ul></div><div className="signin-actions"><a className="button button-primary" href="/signin-with-chatgpt?return_to=%2F">SIGN IN WITH CHATGPT →</a><button className="button button-paper" onClick={continueBuilding}>CONTINUE AS GUEST</button></div><small>No paid plan is required by this builder. Project data is scoped to the signed-in identity.</small></div>}
...
export default function Home() {
  ...
  const [cloudStatus,setCloudStatus]=useState<"idle"|"loading"|"ready"|"signin"|"error"|"session">("idle");
  const [account,setAccount]=useState<Account|null>(null);
  const [dirty,setDirty]=useState(true);
  const [durableHost,setDurableHost]=useState(true);
  ...
  useEffect(()=>{const timer=window.setTimeout(()=>setDurableHost(window.location.hostname.endsWith("chatgpt.site")),0);return()=>window.clearTimeout(timer)},[]);
  ...
  async function loadProjects(){
    if(!durableHost){setCloudStatus("session");setProjects([]);return;}
  ...
  const saveProject=async()=>{
    if(!result)return;
    if(!durableHost){setCloudStatus("session");setViewState("projects");flash("This Vercel mirror is session-only. Export here or save on the full Sites version.");return;}
    try{
  ...
  const rendered=(()=>{
    ...
    if(view==="projects")return <ProjectsView result={result} projects={projects} status={cloudStatus} account={account} save={saveProject} openPlan={openProject} duplicate={duplicateProject} remove={removeProject} refresh={loadProjects} continueBuilding={()=>setView("builder")}/>;
